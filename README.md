...
Registered custom syntax is currently a proving surface rather than broad MLIR dialect coverage. The built-in proving registry covers a fixed subset of Builtin, Func, Arith, and CF operations. Declarative `stablehlo`, `tosa`, `scf`, `linalg`, `acc`, `affine`, `amdgpu`, `amx`, `arith`, `arm_neon`, `arm_sme`, `arm_sve`, `async`, `bufferization`, `cf`, `complex`, and `emitc` presets expose common operands, results, named clauses, regions, successors, and block arguments for structural queries; ArmNeon covers 1 of its 7 operations, ArmSME covers 3 of its 68 operations, ArmSVE covers 9 of its 21 custom-form operations, Async covers 11 of its 29 operations, Bufferization covers 5 of its 7 operations, CF covers 2 of its 4 operations, Complex covers default forms of 21 of its 29 operations, and EmitC covers 20 of its 45 custom-form operations. The `dlti` preset adds core syntax but no operation registrations: LLVM 22.1 DLTI defines six attributes and no operations or types, and those attributes remain lossless opaque values. Presets do not implement dialect verification or execution. Zirium does not load LLVM dialect definitions, interpret arbitrary ODS/TableGen files, or run Python callbacks while parsing.
