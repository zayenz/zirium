"""Lossless MLIR syntax and mutable semantic documents backed by Zirium."""

from collections.abc import Mapping
from os import PathLike
from types import TracebackType
from typing import Any, Literal, Self, TypeAlias, TypeVar

from .config import OperationAlternativesConfig as OperationAlternativesConfig
from .config import OperationFormatConfig as OperationFormatConfig
from .config import OperationGrammarConfig as OperationGrammarConfig
from .config import (
    OperationShapeConfig as OperationShapeConfig,
)
from .config import (
    RegistryConfig as RegistryConfig,
)
from .query import QueryExpr

_QueryResult = TypeVar("_QueryResult")

__version__: str

_TextRange: TypeAlias = tuple[int, int]
_Path: TypeAlias = str | PathLike[str]
_Retention: TypeAlias = Literal[
    "semantic", "semantic_only", "syntax", "syntax_only", "hybrid"
]
_SemanticDiagnosticCode: TypeAlias = Literal[
    "semantic.Syntax",
    "semantic.ResourceLimit",
    "semantic.DuplicateDefinition",
    "semantic.UnresolvedReference",
    "semantic.ArityMismatch",
    "semantic.Type",
    "semantic.Attribute",
    "semantic.Location",
    "semantic.Affine",
    "semantic.ControlFlow",
    "semantic.Symbol",
    "semantic.InvalidValue",
]
_DeclarativeOperation: TypeAlias = Literal[
    "builtin.module",
    "func.func",
    "func.return",
    "func.call",
    "arith.constant",
    "arith.addi",
    "cf.br",
    "cf.cond_br",
]

class StaleHandleError(Exception): ...
class ForeignHandleError(Exception): ...
class SemanticEditError(Exception): ...
class StructuralVerificationError(Exception): ...
class SemanticVerificationError(Exception): ...

class Diagnostic:
    @property
    def kind(self) -> str: ...
    @property
    def message(self) -> str: ...
    @property
    def range(self) -> _TextRange: ...

class OperationShape:
    FUNC_LIKE: OperationShape
    CALL_LIKE: OperationShape
    BINARY_OPERANDS: OperationShape
    OPTIONAL_TYPED_OPERANDS: OperationShape
    ATTR_FIRST_OPTIONAL_TYPED_OPERANDS: OperationShape
    UNARY_OPERAND: OperationShape
    VARIADIC_OPERANDS: OperationShape
    LITERAL_ATTRIBUTE: OperationShape
    OPERAND_CLAUSES: OperationShape
    REGION_CLAUSES: OperationShape

class DialectRegistry:
    """Dialect syntax and semantic rules retained through parse and lower."""

    @staticmethod
    def preset_names() -> tuple[str, ...]: ...
    def operation_names(self) -> tuple[str, ...]: ...
    def operation_shape(self, name: str) -> str | None: ...
    def operation_format(self, name: str) -> str | None: ...
    def call_target_attribute(self, name: str) -> str | None: ...
    def operation_alternatives(
        self, name: str
    ) -> list[tuple[Literal["shape", "format"], str]] | None: ...
    @staticmethod
    def from_name(name: str) -> DialectRegistry: ...
    @staticmethod
    def from_config(
        config: RegistryConfig | dict[str, Any],
        *additional_configs: RegistryConfig | dict[str, Any],
    ) -> DialectRegistry: ...
    @staticmethod
    def from_file(
        path: _Path,
        *additional_paths: _Path,
        max_depth: int | None = None,
        max_files: int | None = None,
        max_edges: int | None = None,
        max_bytes: int | None = None,
    ) -> DialectRegistry: ...
    @staticmethod
    def from_package_resources(
        package: str,
        resource: str,
        *additional_resources: str,
        max_depth: int | None = None,
        max_files: int | None = None,
        max_edges: int | None = None,
        max_bytes: int | None = None,
    ) -> DialectRegistry: ...
    @staticmethod
    def empty() -> DialectRegistry: ...
    @staticmethod
    def core() -> DialectRegistry: ...
    @staticmethod
    def baseline() -> DialectRegistry: ...
    @staticmethod
    def declarative(operations: list[_DeclarativeOperation]) -> DialectRegistry: ...
    @staticmethod
    def with_operation_shapes(
        operation_shapes: Mapping[str, OperationShape],
    ) -> DialectRegistry: ...
    def extend_operation_shapes(
        self, operation_shapes: Mapping[str, OperationShape]
    ) -> DialectRegistry: ...

class SyntaxNode:
    @property
    def kind(self) -> str: ...
    @property
    def range(self) -> _TextRange | None: ...
    @property
    def has_error(self) -> bool: ...
    def child_indices(self) -> bytes: ...
    def descendant_range(self) -> tuple[int, int]: ...
    def token_range(self) -> tuple[int, int]: ...
    def text(self) -> bytes: ...
    def as_operation(self) -> Operation | None: ...

class Token:
    @property
    def kind(self) -> str: ...
    @property
    def range(self) -> _TextRange: ...
    def text(self) -> bytes: ...

class SyntaxTable:
    """Packed native-endian columns whose indexes and kind codes belong to one source File."""
    @property
    def node_kind(self) -> bytes: ...
    @property
    def node_start(self) -> bytes: ...
    @property
    def node_end(self) -> bytes: ...
    @property
    def node_subtree_end(self) -> bytes: ...
    @property
    def node_flags(self) -> bytes: ...
    @property
    def token_kind(self) -> bytes: ...
    @property
    def token_start(self) -> bytes: ...
    @property
    def token_end(self) -> bytes: ...
    @staticmethod
    def node_kind_code(name: str) -> int: ...
    @staticmethod
    def node_kind_name(code: int) -> str: ...
    @staticmethod
    def token_kind_code(name: str) -> int: ...
    @staticmethod
    def token_kind_name(code: int) -> str: ...

class Operation:
    @property
    def range(self) -> _TextRange | None: ...
    @property
    def has_error(self) -> bool: ...
    def syntax(self) -> SyntaxNode: ...
    def text(self) -> bytes: ...
    def properties(self) -> SyntaxNode | None: ...
    def attributes(self) -> SyntaxNode | None: ...
    def trailing_location(self) -> SyntaxNode | None: ...

class SyntaxOperationTable:
    @property
    def operation_node(self) -> bytes: ...
    @property
    def result_offsets(self) -> bytes: ...
    @property
    def result_nodes(self) -> bytes: ...
    @property
    def operand_offsets(self) -> bytes: ...
    @property
    def operand_nodes(self) -> bytes: ...
    @property
    def successor_offsets(self) -> bytes: ...
    @property
    def successor_nodes(self) -> bytes: ...
    @property
    def region_offsets(self) -> bytes: ...
    @property
    def region_nodes(self) -> bytes: ...

class File:
    """Owning lossless parse result with syntax diagnostics and lowering entry points."""

    @property
    def root(self) -> SyntaxNode: ...
    @property
    def diagnostics(self) -> list[Diagnostic]: ...
    def line_column(self, offset: int) -> tuple[int, int]:
        """Convert a byte offset to a one-based ``(line, column)`` pair."""
    def original_bytes(self) -> bytes: ...
    def write_original(self, path: _Path) -> None: ...
    def formatted_bytes(self, width: int = 100, indent: int = 2) -> bytes: ...
    def write_formatted(
        self, path: _Path, width: int = 100, indent: int = 2
    ) -> None: ...
    @property
    def node_count(self) -> int: ...
    @property
    def token_count(self) -> int: ...
    def node(self, index: int) -> SyntaxNode: ...
    def token(self, index: int) -> Token: ...
    def syntax_table(self) -> SyntaxTable: ...
    @property
    def operation_count(self) -> int: ...
    def operation(self, index: int) -> Operation: ...
    def operation_table(self) -> SyntaxOperationTable: ...
    def lower_strict(self, retention: _Retention | None = None) -> LoweringResult: ...
    def lower_best_effort(
        self, retention: _Retention | None = None
    ) -> LoweringResult: ...

class LoweringResult:
    @property
    def document(self) -> Document | None: ...
    @property
    def diagnostics(self) -> list[SemanticDiagnostic]: ...
    @property
    def semantically_complete(self) -> bool: ...

class SemanticDiagnostic:
    @property
    def kind(self) -> _SemanticDiagnosticCode: ...
    @property
    def code(self) -> _SemanticDiagnosticCode: ...
    @property
    def range(self) -> _TextRange: ...
    @property
    def message(self) -> str: ...

class SemanticStatistics:
    @property
    def operations(self) -> int: ...
    @property
    def regions(self) -> int: ...
    @property
    def blocks(self) -> int: ...
    @property
    def local_types(self) -> int: ...
    @property
    def local_attributes(self) -> int: ...
    @property
    def payload_blobs(self) -> int: ...
    @property
    def payload_blob_bytes(self) -> int: ...
    @property
    def retained_source_bytes(self) -> int: ...
    @property
    def direct_owned_bytes(self) -> int: ...
    @property
    def document_index_bytes(self) -> int: ...
    @property
    def retained_cst_bytes(self) -> int: ...
    @property
    def source_storage_shared(self) -> bool: ...
    @property
    def cst_storage_shared(self) -> bool: ...
    @property
    def pooled_list_entries(self) -> int: ...
    @property
    def use_index_entries(self) -> int: ...
    @property
    def symbol_index_entries(self) -> int: ...
    @property
    def dominance_index_entries(self) -> int: ...

class OperationTable:
    @property
    def revision(self) -> int: ...
    @property
    def name_code(self) -> bytes: ...
    @property
    def source_start(self) -> bytes: ...
    @property
    def source_end(self) -> bytes: ...
    @property
    def root_flags(self) -> bytes: ...
    @property
    def name_offsets(self) -> bytes: ...
    @property
    def name_bytes(self) -> bytes: ...
    @property
    def count(self) -> int: ...
    def operation(self, index: int) -> SemanticOperation: ...

class Document:
    """Mutable semantic document owning generation-checked handles.

    Queries return snapshots or live document-scoped wrappers. A normal edit
    context exit publishes its buffered commands atomically; surviving handles
    retain identity and erased handles become stale.
    """

    @property
    def semantically_complete(self) -> bool: ...
    @property
    def retention(self) -> Literal["semantic", "syntax", "hybrid"]: ...
    @property
    def diagnostics(self) -> list[SemanticDiagnostic]: ...
    def query(
        self,
        expression: QueryExpr[_QueryResult],
        *,
        max_work: int | None = None,
        max_items: int | None = None,
        strict: bool = False,
    ) -> _QueryResult:
        """Evaluate a reusable expression, subject to optional work/item limits."""
    def operation_table(self, name: str | None = None) -> OperationTable: ...
    def statistics(self) -> SemanticStatistics: ...
    def edit(self) -> SemanticEdit:
        """Create a buffered transaction to use as a context manager."""
    def uses(self, value: SemanticValue) -> list[SemanticUse]: ...
    def lookup_symbol(
        self, from_operation: SemanticOperation, symbol: str
    ) -> SemanticOperation | None: ...
    def symbol_diagnostics(self) -> list[tuple[SemanticOperation, str]]: ...
    def dominates(self, value: SemanticValue, operation: SemanticOperation) -> bool: ...
    def validate_structure(self) -> None: ...
    def verify_semantics(self) -> None: ...
    def formatted_bytes(
        self,
        assembly: Literal["source", "generic"] = "source",
        width: int = 100,
        indent: int = 2,
    ) -> bytes: ...
    def write_formatted(
        self,
        path: _Path,
        assembly: Literal["source", "generic"] = "source",
        width: int = 100,
        indent: int = 2,
    ) -> None: ...
    def canonical_bytes(self, compact: bool = False) -> bytes: ...
    def write_canonical(self, path: _Path, compact: bool = False) -> None: ...
    def custom_bytes(self, compact: bool = False) -> bytes: ...
    def write_custom(self, path: _Path, compact: bool = False) -> None: ...
    # Raises ValueError after a committed Hybrid insertion or erasure: those
    # structural edits clear retained source/CST and syntax mappings.
    def preserving_bytes(self, compact: bool = False) -> bytes: ...
    def write_preserving(self, path: _Path, compact: bool = False) -> None: ...
    def structurally_equal(self, other: Document) -> bool: ...

class AttributeSpecHandle:
    def __init__(
        self, attribute: SemanticAttribute, name: str | None = None
    ) -> None: ...
    @property
    def name(self) -> str: ...

class OperationSpec:
    def __init__(
        self,
        name: str,
        operands: list[SemanticValue],
        result_types: list[SemanticType],
        function_type: SemanticType,
        attributes: list[AttributeSpecHandle] = [],
        properties: list[AttributeSpecHandle] = [],
    ) -> None: ...

class SemanticEdit:
    """Buffered transaction that atomically commits on normal context exit."""

    def __enter__(self) -> Self: ...
    def __exit__(
        self,
        exc_type: type[BaseException] | None,
        exc_value: BaseException | None,
        traceback: TracebackType | None,
    ) -> bool: ...
    # On Hybrid documents, insertion clears retained source/CST and syntax
    # mappings when committed; preserving output then raises ValueError.
    def insert_root(self, index: int, spec: OperationSpec) -> None: ...
    # On Hybrid documents, insertion clears retained source/CST and syntax
    # mappings when committed; preserving output then raises ValueError.
    def insert(self, block: SemanticBlock, index: int, spec: OperationSpec) -> None: ...
    def erase(self, operation: SemanticOperation) -> None:
        """Queue erasure; on commit the handle becomes stale.

        The operation must be regionless, have no live uses, and belong to this
        document. Hybrid erasure disables preserving output; canonical output
        remains available.
        """
    def rewire_operand(
        self, operation: SemanticOperation, index: int, value: SemanticValue
    ) -> None: ...
    def rewire_successor_argument(
        self,
        operation: SemanticOperation,
        successor: int,
        argument: int,
        value: SemanticValue,
    ) -> None: ...
    def replace_result_types(
        self, operation: SemanticOperation, types: list[SemanticType]
    ) -> None: ...
    def set_attribute(
        self, operation: SemanticOperation, value: AttributeSpecHandle
    ) -> None: ...
    def remove_attribute(self, operation: SemanticOperation, name: str) -> None: ...
    def set_property(
        self, operation: SemanticOperation, value: AttributeSpecHandle
    ) -> None: ...
    def remove_property(self, operation: SemanticOperation, name: str) -> None: ...
    def replace_all_uses(self, from_: SemanticValue, to: SemanticValue) -> None: ...
    def compact_pools(self) -> None: ...

class SemanticUse:
    @property
    def kind(self) -> Literal["operand", "successor_argument"]: ...
    @property
    def operation(self) -> SemanticOperation: ...
    @property
    def index(self) -> int: ...
    @property
    def successor(self) -> int | None: ...

class SemanticOperation:
    """Document-owned live operation identity.

    Equal lookups hash alike and surviving handles reflect edits. Reading an
    erased operation raises ``StaleHandleError``; cross-document edits raise
    ``ForeignHandleError``.
    """
    @property
    def mnemonic(self) -> str: ...
    @property
    def name(self) -> str: ...
    @property
    def symbol_name(self) -> str | None: ...
    @property
    def signature(self) -> str | None: ...
    @property
    def callee(self) -> str | None: ...
    @property
    def callee_segments(self) -> list[str] | None: ...
    @property
    def callee_spelling(self) -> str | None: ...
    @property
    def source_range(self) -> _TextRange | None: ...
    @property
    def is_unparsed(self) -> bool: ...
    @property
    def unparsed_text(self) -> bytes | None: ...
    def region_count(self) -> int: ...
    def region(self, index: int) -> SemanticRegion: ...
    def operand_count(self) -> int: ...
    def operand(self, index: int) -> SemanticValue: ...
    def result_count(self) -> int: ...
    def result(self, index: int) -> SemanticValue: ...
    def result_type(self, index: int) -> SemanticType: ...
    def function_type(self) -> SemanticType: ...
    def attribute_count(self) -> int: ...
    def attribute(self, index: int) -> SemanticAttribute: ...
    def attribute_by_name(self, name: str) -> SemanticAttribute | None: ...
    def attribute_snapshot(self) -> list[tuple[str, str]]: ...
    def property_snapshot(self) -> list[tuple[str, str]]: ...

class SemanticRegion:
    def block_count(self) -> int: ...
    def block(self, index: int) -> SemanticBlock: ...
    def parent_operation(self) -> SemanticOperation: ...

class SemanticBlock:
    @property
    def label(self) -> str | None: ...
    def operation_count(self) -> int: ...
    def operation(self, index: int) -> SemanticOperation: ...
    def argument_count(self) -> int: ...
    def argument(self, index: int) -> SemanticValue: ...
    def parent_region(self) -> SemanticRegion: ...

class SemanticValue:
    @property
    def valid(self) -> bool: ...
    @property
    def kind(self) -> Literal["operation_result", "block_argument", "invalid"]: ...
    @property
    def key(self) -> int | None: ...
    @property
    def defining_operation(self) -> SemanticOperation | None: ...
    @property
    def type_value(self) -> SemanticType | None: ...

class SemanticType:
    """Structured type view. Properties return ``None`` when not applicable."""
    @property
    def spelling(self) -> str: ...
    @property
    def kind(
        self,
    ) -> Literal[
        "integer",
        "float",
        "index",
        "complex",
        "tuple",
        "tensor",
        "vector",
        "memref",
        "function",
        "opaque",
        "invalid",
    ]: ...
    @property
    def integer_width(self) -> int | None: ...
    @property
    def integer_signedness(
        self,
    ) -> Literal["signless", "signed", "unsigned"] | None: ...
    @property
    def float_name(self) -> str | None: ...
    @property
    def dimensions(self) -> list[int | None] | None: ...
    @property
    def scalable_dimensions(self) -> list[bool] | None: ...
    @property
    def unranked(self) -> bool | None: ...
    @property
    def element_type(self) -> SemanticType | None: ...
    @property
    def element_count(self) -> int | None: ...
    def element(self, index: int) -> SemanticType | None: ...
    @property
    def input_count(self) -> int | None: ...
    def input(self, index: int) -> SemanticType | None: ...
    @property
    def result_count(self) -> int | None: ...
    def result(self, index: int) -> SemanticType | None: ...

class SemanticAttribute:
    @property
    def name(self) -> str: ...
    @property
    def kind(
        self,
    ) -> Literal[
        "dense",
        "sparse",
        "resource",
        "boolean",
        "integer",
        "float",
        "string",
        "type",
        "symbol",
        "array",
        "dictionary",
        "location",
        "affine_map",
        "integer_set",
        "wide_number",
        "opaque",
        "invalid",
    ]: ...
    @property
    def spelling(self) -> str: ...
    @property
    def string_value(self) -> str | None: ...
    @property
    def integer_value(self) -> int | None: ...
    @property
    def float_value(self) -> float | None: ...
    @property
    def boolean_value(self) -> bool | None: ...
    @property
    def symbol_value(self) -> str | None: ...
    @property
    def symbol_segments(self) -> list[str] | None: ...
    @property
    def element_count(self) -> int | None: ...
    def element(self, index: int) -> SemanticAttribute | None: ...
    @property
    def payload_byte_length(self) -> int | None: ...
    def raw_buffer(self) -> bytes | None: ...

class ResourceLimitError(Exception): ...

class Change:
    @property
    def kind(self) -> Literal["added", "removed", "modified", "moved"]: ...
    @property
    def moved(self) -> bool: ...
    @property
    def before(self) -> SemanticOperation | None: ...
    @property
    def after(self) -> SemanticOperation | None: ...
    @property
    def fields(self) -> list[str]: ...
    @property
    def details(self) -> list[dict[str, Any]]: ...

class Diff:
    @property
    def changes(self) -> list[Change]: ...
    @property
    def compare_locations(self) -> bool: ...
    @property
    def statistics(self) -> dict[str, int]: ...
    def to_json(self) -> str: ...
    def to_text(self) -> str: ...
    def query(
        self,
        expression: QueryExpr[_QueryResult],
        *,
        max_work: int | None = None,
        max_items: int | None = None,
        strict: bool = False,
    ) -> _QueryResult: ...
    def __len__(self) -> int: ...

def diff(
    before: Document,
    after: Document,
    *,
    compare_locations: bool = False,
    max_work: int | None = None,
    max_changes: int | None = None,
) -> Diff: ...
def parse_bytes(
    data: bytes,
    *,
    registry: DialectRegistry | None = None,
    max_file_bytes: int | None = None,
    max_tokens: int | None = None,
    max_delimiter_depth: int | None = None,
    max_payload_bytes: int | None = None,
    max_numeric_literal_bytes: int | None = None,
    max_attribute_depth: int | None = None,
    max_alias_expansion_depth: int | None = None,
) -> File: ...
def parse_text(
    text: str,
    *,
    registry: DialectRegistry | None = None,
    max_file_bytes: int | None = None,
    max_tokens: int | None = None,
    max_delimiter_depth: int | None = None,
    max_payload_bytes: int | None = None,
    max_numeric_literal_bytes: int | None = None,
    max_attribute_depth: int | None = None,
    max_alias_expansion_depth: int | None = None,
) -> File: ...
def parse_file(
    path: _Path,
    *,
    registry: DialectRegistry | None = None,
    max_file_bytes: int | None = None,
    max_tokens: int | None = None,
    max_delimiter_depth: int | None = None,
    max_payload_bytes: int | None = None,
    max_numeric_literal_bytes: int | None = None,
    max_attribute_depth: int | None = None,
    max_alias_expansion_depth: int | None = None,
) -> File: ...
